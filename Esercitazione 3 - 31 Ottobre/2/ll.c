...
llNode *createNode(int num_queries, int *queries, int *queries_results){
        
        llNode *node = malloc(sizeof(llNode));
        node->next = NULL;
        node->num_queries = num_queries;
        node->queries = queries;
        node->queries_results = queries_results;
        return node;
    }
